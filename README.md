...
social.html         — דשבורד: הסרטונים הרשמיים מיוטיוב לצד פוסטים מפייסבוק, ואינסטגרם
press.html          — כתבו עלינו: כתבות טלוויזיה, פודקאסטים ועיתונות
...
מערכת הניוזלטרים מציעה שמירה של גיליונות יציבים בכתובות קבועות (למשל `newsletter.html?id=newsletter-2026-06`), תמיכה ב-4 תבניות תצוגה מעוצבות בעלות זהות ויזואלית נפרדת, ואפשרות הדפסה נקייה או שמירה כ-PDF ישירות מהדפדפן.

#### ארבע התבניות (`template`)
| מפתח נתונים | שם בעברית | אופי עיצובי |
|-------------|-----------|-------------|
| `classic` | קלאסי־מערכתי | מגזין חינוכי אלגנטי ורגוע: כותרת סריף גדולה, פתיח כ"ציטוט עורך", וכתבות בשורות עם תמונות לסירוגין. |
| `magazine` | מגזיני | גיליון דיגיטלי ויזואלי: כתבת שער (Hero) עם תמונה גדולה וכותרת על גבי התמונה, ושאר הכתבות ברשת כרטיסים. |
| `poster` | חממתי / פוסטר | זין יצירתי בנייר: בלוקים נועזים, מספור מקטעים (01, 02...), מדבקות וצללים מודפסים בירוק/ניאון. |
| `compact` | גרסת הדפסה | מהדורה להדפסה: עמודה צרה בסגנון A4, טיפוגרפיה קומפקטית, מעברי עמוד נקיים וחיסכון בנייר. |

> בורר התבניות בכלי הצוות (`newsletter-preview.html`) מציג את השמות החדשים בעברית, אך הערכים הנשמרים בגיליון (`classic`, `magazine`, `poster`, `compact`) נותרו ללא שינוי כדי לא לשבור ניוזלטרים קיימים.
...
3. **שיתוף הניוזלטר**:
   ...
   - הדף הציבורי **אינו** מכיל עוד כפתורי "העתק קישור" או "העתק טקסט למייל" — הוא מעוצב כדף מגזין/ניוזלטר דיגיטלי ולא ככלי עזר.
   - לשליחה בדיוור: כתבו במייל הודעה קצרה משלכם (ברכה + הקשר), הוסיפו את כתובת הניוזלטר (`newsletter.html?id=[מזהה]`) ושלחו דרך Gmail, Brevo, Mailchimp או כל תוכנת דיוור אחרת.
4. **הדפסה ושמירה כ-PDF**:
   - לחצו על כפתור **"הדפסה / שמירה כ־PDF"** הממוקם **בתחתית** דף הניוזלטר (אחרי כל הכתבות, לפני הפוטר). הכפתור עצמו מוסתר בתצוגת ההדפסה.
...
- [ ] ודאו שאין בדף הציבורי כפתורי "העתק קישור" / "העתק טקסט למייל" — נותר רק כפתור **הדפסה / שמירה כ־PDF** בתחתית הדף.
- [ ] בדקו את התנהגות ההדפסה ולוח התצוגה המקדימה: ודאו שהכפתור והניווט מוסתרים, שהתמונות אינן נחתכות ושמעברי העמוד סבירים.
