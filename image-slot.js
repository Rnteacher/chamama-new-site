...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    static get observedAttributes() {
      return ['shape', 'radius', 'mask', 'fit', 'position', 'placeholder', 'src', 'id', 'zoom', 'panx', 'pany'];
    }
    ...
    _render() {
      ...
      if (!this.hasAttribute('data-reframe')) {
        // Fallback crop from attributes (zoom/panx/pany) when there's no
        // stored sidecar value — lets a baked `src` keep its framing without
        // depending on the .image-slots.state.json sidecar (e.g. exported site).
        const av = (n, d) => { const v = parseFloat(this.getAttribute(n)); return Number.isFinite(v) ? v : d; };
        this._view = {
          s: stored && Number.isFinite(stored.s) ? clampS(stored.s) : clampS(av('zoom', 1)),
          x: stored && Number.isFinite(stored.x) ? stored.x : av('panx', 0),
          y: stored && Number.isFinite(stored.y) ? stored.y : av('pany', 0),
        };
      }
